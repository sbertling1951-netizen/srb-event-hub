-- Account Activation: recognize an already-activated account instead of
-- driving another activation.
--
-- evaluate_member_identity_claim already computes has_active_auth_account
-- (EXISTS an active public.person_auth_accounts row for the resolved
-- Person) and already sets review_reason = 'EXISTING_AUTH_RELATIONSHIP_PRESENT'
-- for a single strong-evidence candidate in that state -- but
-- public_result_classification had no value for it and fell through to
-- 'CONTINUE_VERIFICATION', so Account Activation showed "Finish Activating"
-- and let an already-activated member proceed toward a redundant
-- activation (and, if they entered an email other than their account
-- email, a second auth user + a second person_auth_accounts row).
--
-- This adds one CASE arm to public_result: a single candidate
-- (cs.candidate_count = 1, so sc is populated) whose Person has an active
-- auth account now classifies as 'ALREADY_ACTIVATED'. The activation UI
-- shows "Your account is already activated. Please sign in to continue."
-- with a Sign In action; recovery stays available on the sign-in page.
--
-- No identity-threshold, candidate-resolution, scoring, event-evidence, or
-- reconciliation-contract change. Anti-enumeration is unchanged: this arm
-- is only reachable at the existing single-strong-candidate bar
-- (matched_candidates requires an email/phone/membership match; weak or
-- ambiguous evidence yields candidate_count 0 or > 1). Every downstream
-- RPC already gates on public_result_classification = 'CONTINUE_VERIFICATION'
-- (begin_member_identity_claim_verification and
-- finalize_member_identity_activation both fail closed otherwise), so
-- returning 'ALREADY_ACTIVATED' halts the workflow with no mutation --
-- no finalize / magic-link / verification RPC needs to change.
--
-- identity_claim_attempts.public_result_classification carries a CHECK
-- whitelist; this widens it (additively) to accept the new value. The
-- internal_result_classification / candidate_count_classification / review_reason
-- whitelists are unchanged -- this case still records
-- internal_result = 'UNIQUE_CANDIDATE', candidate_count = 'ONE_PERSON',
-- review_reason = 'EXISTING_AUTH_RELATIONSHIP_PRESENT', all already allowed.

BEGIN;

ALTER TABLE public.identity_claim_attempts
  DROP CONSTRAINT identity_claim_attempts_public_result_classification_check;

ALTER TABLE public.identity_claim_attempts
  ADD CONSTRAINT identity_claim_attempts_public_result_classification_check
  CHECK (public_result_classification = ANY (ARRAY[
    'CONTINUE_VERIFICATION'::text,
    'REVIEW_REQUIRED'::text,
    'CREATE_NEW_ACCOUNT_AVAILABLE'::text,
  ...
  ELSE
    ...
    SELECT
      ...
      CASE
        ...
        -- Already-activated recognition (intended-but-missing branch).
        -- A single, strong-evidence candidate (matched_candidates already
        -- requires an email/phone/membership match) whose canonical Person
        -- holds an active auth account is already activated. Same
        -- disclosure strength as the existing single-candidate result:
        -- weak/ambiguous evidence never reaches here (candidate_count would
        -- be 0 or > 1, or sc would be NULL). Downstream steps
        -- (begin_member_identity_claim_verification,
        -- finalize_member_identity_activation) already refuse any
        -- public_result_classification other than 'CONTINUE_VERIFICATION',
        -- so this value alone stops the workflow with zero identity/account
        -- mutation. review_reason is already 'EXISTING_AUTH_RELATIONSHIP_PRESENT'
        -- for this case; internal_result stays 'UNIQUE_CANDIDATE'.
        WHEN sc.has_active_auth_account THEN 'ALREADY_ACTIVATED'
