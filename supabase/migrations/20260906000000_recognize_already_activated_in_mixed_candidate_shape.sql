-- Account Activation: already-activated recognition also covers the
-- MIXED candidate shape (the shape production actually hits).
--
-- 20260905000000 added the ALREADY_ACTIVATED public result but gated it on
-- cs.candidate_count = 1. In production a real already-activated member
-- typically ALSO has an unresolved (person_id IS NULL) attendee row for a
-- newer registration carrying the same name/email/phone -- so
-- evaluate_member_identity_claim returns candidate_count = 2
-- (candidate_count_classification 'MIXED': one resolved Person + one
-- unresolved component) and classified as 'REVIEW_REQUIRED'. The activation
-- page renders no action for REVIEW_REQUIRED, so the member is stranded on
-- the form -- exactly the reported "pressing Continue does nothing" symptom.
--
-- Fix: a single_activated_person CTE selects the one resolved Person
-- candidate when it is the ONLY resolved Person among the matched
-- candidates (person_candidate_count = 1) and it already holds an active
-- auth account. public_result returns 'ALREADY_ACTIVATED' whenever that
-- CTE is non-empty -- before the candidate_count > 1 check -- so the MIXED
-- (1 person + N unresolved components) shape resolves to "already
-- activated" instead of stranding on review. Two distinct resolved Persons
-- (person_candidate_count >= 2) still fall through to 'REVIEW_REQUIRED'.
--
-- Unchanged: candidate resolution, name/identifier matching, evidence
-- thresholds, internal_result, event evidence, and every mutation path.
-- The unresolved components are neither linked, disclosed, nor mutated
-- here. Anti-enumeration is unchanged -- still the single-resolved-Person +
-- strong-identifier-match bar. Downstream RPCs already fail closed on any
-- public_result_classification other than 'CONTINUE_VERIFICATION', so
-- 'ALREADY_ACTIVATED' performs zero identity/account mutation. No CHECK
-- constraint change (public_result whitelist already carries the value
-- from 20260905000000; review_reason has no CHECK).

BEGIN;

CREATE OR REPLACE FUNCTION public.evaluate_member_identity_claim(p_first_name text, p_last_name text, p_home_state text DEFAULT NULL::text, p_email text DEFAULT NULL::text, p_phone text DEFAULT NULL::text, p_membership_number text DEFAULT NULL::text, p_event_ids uuid[] DEFAULT NULL::uuid[], p_request_ip_hash text DEFAULT NULL::text, p_user_agent_hash text DEFAULT NULL::text, p_request_source text DEFAULT 'member_activation_api'::text)
 RETURNS TABLE(attempt_id uuid, public_attempt_token text, internal_result_classification text, public_result_classification text, candidate_count_classification text, matched_person_id uuid, matched_component_id text, review_reason text, status text, expires_at timestamp with time zone)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  ELSE
    ...
    single_activated_person AS (
      -- The one resolved Person candidate, when it is the ONLY resolved
      -- Person among the matched candidates and it already holds an active
      -- auth account. Any UNRESOLVED_COMPONENT candidates that also matched
      -- the same strong evidence can only ever resolve TO a Person, and
      -- there is exactly one resolved-Person option here -- so "already
      -- activated" is the honest answer even in the MIXED (1 person + N
      -- components) shape, which otherwise strands the member on
      -- REVIEW_REQUIRED. Excludes person_candidate_count >= 2 (two distinct
      -- Persons -> genuine ambiguity -> still REVIEW_REQUIRED).
      SELECT mc.person_id
      FROM matched_candidates mc
      CROSS JOIN candidate_summary cs
      WHERE cs.person_candidate_count = 1
        AND mc.candidate_kind = 'PERSON'
        AND mc.has_active_auth_account
      LIMIT 1
    )
    SELECT
      CASE
      ...
      CASE
        ...
        -- Already-activated recognition. Exactly one resolved Person
        -- candidate, matched on strong evidence (matched_candidates already
        -- requires an email/phone/membership match), and it already holds an
        -- active auth account. Reached even when unresolved components also
        -- matched (candidate_count_classification MIXED) -- those can only
        -- resolve to that same Person. NOT reached when two distinct
        -- Persons match (person_candidate_count >= 2 -> sap is empty ->
        -- REVIEW_REQUIRED below). Every downstream step
        -- (begin_member_identity_claim_verification,
        -- finalize_member_identity_activation, the matched-person fast path)
        -- already refuses any public_result_classification other than
        -- 'CONTINUE_VERIFICATION', so this value alone halts activation with
        -- zero identity/account mutation.
        WHEN sap.person_id IS NOT NULL THEN 'ALREADY_ACTIVATED'
      ...
      coalesce(sc.person_id, sap.person_id) AS person_id,
      sc.component_id,
      CASE
        WHEN cs.candidate_count = 0 THEN 'NO_MATCHING_HISTORICAL_EVIDENCE'
        WHEN sap.person_id IS NOT NULL THEN 'EXISTING_AUTH_RELATIONSHIP_PRESENT'
    ...
    LEFT JOIN single_activated_person sap ON true;
  END IF;
