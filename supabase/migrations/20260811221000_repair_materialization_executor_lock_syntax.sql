-- Repair invalid SQL in the materialization executor found during dry-run
-- validation: `FOR UPDATE` cannot be combined with an aggregate function
-- (array_agg) in the same query. Splits row-locking (PERFORM ... FOR
-- UPDATE, in deterministic id order) from the subsequent aggregation used
-- to compare against the approved manifest. No other logic changes.
BEGIN;
...
BEGIN
  ...
  -- further check. FOR UPDATE cannot be combined with array_agg in one
  -- query, so locking and aggregation are two separate statements.
  PERFORM id FROM public.admin_event_access
