...
// Boolean answers are compared EXACTLY after key() (trim, case-fold, collapse
// whitespace, straight/curly quote normalization). No prefix, substring or fuzzy
// matching: any other nonempty answer is malformed_boolean, never a silent No.
const BOOLEAN_YES = ["yes", "y", "true", "1"] as const, BOOLEAN_NO = ["no", "n", "false", "0"] as const;
// The registration platform's two established full answers to the email-sharing
// question. They are consent evidence, so they are recognized exactly and only
// for share_with_attendees; the raw answer text stays unchanged in source_payload.
const SHARING_YES = [...BOOLEAN_YES, "yes, share my email"] as const, SHARING_NO = [...BOOLEAN_NO, "no. don't share my email"] as const;
function bool(v: unknown, label: string, issues: ImportIssue[], yes: readonly string[] = BOOLEAN_YES, no: readonly string[] = BOOLEAN_NO) { const s=key(text(v)); if (!s) return false; if (yes.includes(s)) return true; if (no.includes(s)) return false; issues.push({code:"malformed_boolean",message:`Malformed ${label}`,severity:"error"}); return false; }
