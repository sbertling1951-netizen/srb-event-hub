...
export function useMapInteraction(args: Args) {
  ...
  useEffect(() => {
    ...
    if (!el || !enabled || selectionMode === "none" || selectionMode === "single") {
      return;
    }
    ...
    const toPct = (clientX: number, clientY: number): MapPercentPoint => {
      ...
      const { xPct, yPct } = screenToPercent(sx, sy, a().getTransform(), a().natural);
      return { xPct, yPct };
    };
    ...
    const onPointerDown = (e: PointerEvent) => {
      ...
      if (isOnMarker(e.target)) return; // marker handled by MarkerLayer
      // two-finger / non-primary touch -> let V2 pinch
      pointerId = e.pointerId;
      startScreen = { x: e.clientX, y: e.clientY };

      const shift = (e as PointerEvent & { shiftKey?: boolean }).shiftKey;
      const desktopMarquee = e.pointerType === "mouse" && (shift || selectionMode === "rectangle");
      const touchMarquee = e.pointerType !== "mouse";

      if (desktopMarquee) {
        // claim immediately on desktop
        try {
      ...
      } else if (touchMarquee) {
        // arm on long-press; if the finger moves first it becomes a pan (V2)
        clearLongPress();
    ...
    const onPointerMove = (e: PointerEvent) => {
      ...
      if (!armed) {
        // movement before arming = pan intent -> cancel pending long-press, yield to V2
        if (startScreen) {
          const dx = Math.abs(e.clientX - startScreen.x);
          const dy = Math.abs(e.clientY - startScreen.y);
          if (dx > moveThresholdPx || dy > moveThresholdPx) clearLongPress();
        }
        return;
      }

      e.preventDefault();
      if (startPct) a().onMarqueeUpdate?.(startPct, toPct(e.clientX, e.clientY));
    };
