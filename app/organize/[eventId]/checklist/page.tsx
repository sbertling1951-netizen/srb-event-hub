"use client";

import Link from "next/link";
import { useCallback, useEffect, useState } from "react";

import { OrganizerChecklistFields } from "@/components/organize/OrganizerChecklistFields";
import { Alert } from "@/components/ui/Alert";
import { AppButton } from "@/components/ui/AppButton";
import { Page } from "@/components/ui/Page";
import { PageHeader } from "@/components/ui/PageHeader";
import { PageSection } from "@/components/ui/PageSection";
import {
  addMyPrivateDraftChecklistItem,
  type ChecklistItem,
  checklistItemError,
  type ChecklistItemInput,
  checklistItemValues,
  deleteMyPrivateDraftChecklistItem,
  emptyChecklistItem,
  listMyPrivateDraftChecklistItems,
  updateMyPrivateDraftChecklistItem,
} from "@/lib/organizerChecklist";
import { getMyPrivateEventDraft, type OrganizerDraft } from "@/lib/organizerDrafts";
import { supabase } from "@/lib/supabase";

type ChecklistPageProps = { params: Promise<{ eventId: string }> };
type LoadState = "checking" | "denied" | "missing" | "error" | "ready";

const PRIVACY_COPY =
  "This checklist is yours alone. Nobody else can see it, and ticking things off does not control whether this Event is ready or when it can launch.";

// Deliberately warm and neutral: an empty checklist is a perfectly fine state,
// not a warning, a nag, or an incomplete step. EpicentraX suggests nothing.
const EMPTY_COPY =
  "Nothing here yet — this is a blank page for whatever you want to remember. Add an item below whenever you like.";

export default function OrganizerChecklistPage({ params }: ChecklistPageProps) {
  const [eventId, setEventId] = useState<string>("");
  const [draft, setDraft] = useState<OrganizerDraft | null>(null);
  const [state, setState] = useState<LoadState>("checking");

  const [items, setItems] = useState<ChecklistItem[]>([]);

  const [addForm, setAddForm] = useState<ChecklistItemInput>(emptyChecklistItem());
  const [addOpen, setAddOpen] = useState(false);
  const [addError, setAddError] = useState<string | null>(null);
  const [adding, setAdding] = useState(false);

  const [editingId, setEditingId] = useState<string | null>(null);
  const [editForm, setEditForm] = useState<ChecklistItemInput | null>(null);
  const [editError, setEditError] = useState<string | null>(null);
  const [savingId, setSavingId] = useState<string | null>(null);

  const [busyId, setBusyId] = useState<string | null>(null);
  const [rowError, setRowError] = useState<string | null>(null);

  const loadItems = useCallback(async (id: string) => {
    setItems(await listMyPrivateDraftChecklistItems(supabase, id));
  }, []);

  const load = useCallback(
    async (id: string) => {
      const { data } = await supabase.auth.getSession();
      const user = data.session?.user;
      if (!user || !user.email_confirmed_at) {
        setState("denied");
        return;
      }
      try {
        const ownedDraft = await getMyPrivateEventDraft(supabase, id);
        if (!ownedDraft) {
          setState("missing");
          return;
        }
        setDraft(ownedDraft);
        await loadItems(id);
        setState("ready");
      } catch {
        setState("error");
      }
    },
    [loadItems],
  );

  useEffect(() => {
    void params.then(({ eventId: resolved }) => {
      setEventId(resolved);
      void load(resolved);
    });
  }, [load, params]);

  async function submitAdd(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault();
    const validationError = checklistItemError(addForm);
    if (validationError || adding) {
      setAddError(validationError);
      return;
    }
    setAdding(true);
    setAddError(null);
    try {
      const item = await addMyPrivateDraftChecklistItem(supabase, { eventId, values: addForm });
      setItems((current) => [...current, item]);
      setAddForm(emptyChecklistItem());
      setAddOpen(false);
    } catch (error) {
      setAddError(error instanceof Error ? error.message : "We could not add that item.");
    } finally {
      setAdding(false);
    }
  }

  function startEdit(item: ChecklistItem) {
    setEditingId(item.id);
    setEditForm(checklistItemValues(item));
    setEditError(null);
    setRowError(null);
  }

  function cancelEdit() {
    setEditingId(null);
    setEditForm(null);
    setEditError(null);
  }

  async function submitEdit(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!editingId || !editForm) {
      return;
    }
    const validationError = checklistItemError(editForm);
    if (validationError || savingId) {
      setEditError(validationError);
      return;
    }
    setSavingId(editingId);
    setEditError(null);
    try {
      const item = await updateMyPrivateDraftChecklistItem(supabase, {
        eventId,
        checklistItemId: editingId,
        values: editForm,
      });
      setItems((current) => current.map((existing) => (existing.id === item.id ? item : existing)));
      cancelEdit();
    } catch (error) {
      setEditError(error instanceof Error ? error.message : "We could not save that item.");
    } finally {
      setSavingId(null);
    }
  }

  /** Ticks or unticks one item. A personal marker on that row and nothing else. */
  async function toggleItem(item: ChecklistItem) {
    if (busyId) {
      return;
    }
    setBusyId(item.id);
    setRowError(null);
    try {
      const saved = await updateMyPrivateDraftChecklistItem(supabase, {
        eventId,
        checklistItemId: item.id,
        values: { ...checklistItemValues(item), isCompleted: !item.isCompleted },
      });
      setItems((current) => current.map((existing) => (existing.id === saved.id ? saved : existing)));
    } catch (error) {
      setRowError(error instanceof Error ? error.message : "We could not update that item.");
    } finally {
      setBusyId(null);
    }
  }

  async function removeItem(item: ChecklistItem) {
    if (busyId) {
      return;
    }
    setBusyId(item.id);
    setRowError(null);
    try {
      await deleteMyPrivateDraftChecklistItem(supabase, { eventId, checklistItemId: item.id });
      setItems((current) => current.filter((existing) => existing.id !== item.id));
    } catch (error) {
      setRowError(error instanceof Error ? error.message : "We could not remove that item.");
    } finally {
      setBusyId(null);
    }
  }

  if (state === "checking") {
    return <Page><Alert tone="info">Opening your checklist…</Alert></Page>;
  }
  if (state === "denied") {
    return <Page><Alert tone="warning">Sign in with a verified EpicentraX account to use a checklist.</Alert><p><Link href="/organize">Return to your events</Link></p></Page>;
  }
  if (state === "missing" || !draft) {
    return <Page><Alert tone="warning">This private draft is unavailable. It may belong to a different organizer account.</Alert><p><Link href="/organize">Return to your events</Link></p></Page>;
  }
  if (state === "error") {
    return <Page><Alert tone="danger">We could not open this checklist. Please return to your event and try again.</Alert><p><Link href={`/organize/${encodeURIComponent(eventId)}`}>Back to this event</Link></p></Page>;
  }

  return (
    <Page style={{ maxWidth: 860, margin: "0 auto", display: "grid", gap: 16 }}>
      <PageHeader
        title="Planning checklist"
        headingLevel="h1"
        description={`Planning for “${draft.event_name}” — a private draft.`}
      />
      <Alert tone="info">{PRIVACY_COPY}</Alert>

      {rowError ? <Alert tone="danger">{rowError}</Alert> : null}

      <PageSection title="Your items" variant="section">
        {items.length === 0 ? (
          <Alert tone="neutral">{EMPTY_COPY}</Alert>
        ) : (
          <ul style={{ display: "grid", gap: 10, listStyle: "none", margin: 0, padding: 0 }}>
            {items.map((item) => (
              <li key={item.id} className="card" style={{ display: "grid", gap: 8 }}>
                {editingId === item.id && editForm ? (
                  <form onSubmit={submitEdit} style={{ display: "grid", gap: 12 }}>
                    {editError ? <Alert tone="danger">{editError}</Alert> : null}
                    <OrganizerChecklistFields values={editForm} onChange={setEditForm} />
                    <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                      <AppButton type="submit" variant="primary" loading={savingId === item.id}>Save changes</AppButton>
                      <AppButton type="button" onClick={cancelEdit} disabled={savingId === item.id}>Cancel</AppButton>
                    </div>
                  </form>
                ) : (
                  <>
                    <label style={{ display: "flex", gap: 10, alignItems: "flex-start", fontWeight: 400 }}>
                      <input
                        type="checkbox"
                        checked={item.isCompleted}
                        disabled={busyId === item.id}
                        onChange={() => void toggleItem(item)}
                      />
                      <span style={{ textDecoration: item.isCompleted ? "line-through" : undefined }}>
                        {item.title}
                      </span>
                    </label>
                    {item.targetDate ? (
                      <span style={{ color: "var(--color-text-muted, #475569)" }}>
                        Target date: {item.targetDate}
                      </span>
                    ) : null}
                    {item.organizerNote ? <p style={{ margin: 0 }}>Note: {item.organizerNote}</p> : null}
                    <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                      <AppButton onClick={() => startEdit(item)}>Edit</AppButton>
                      <AppButton
                        variant="danger"
                        loading={busyId === item.id}
                        onClick={() => void removeItem(item)}
                      >
                        Remove
                      </AppButton>
                    </div>
                  </>
                )}
              </li>
            ))}
          </ul>
        )}
      </PageSection>

      <PageSection title="Add an item" variant="card">
        {addOpen ? (
          <form onSubmit={submitAdd} style={{ display: "grid", gap: 12 }}>
            {addError ? <Alert tone="danger">{addError}</Alert> : null}
            <OrganizerChecklistFields values={addForm} onChange={setAddForm} />
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              <AppButton type="submit" variant="primary" loading={adding}>Add item</AppButton>
              <AppButton
                type="button"
                onClick={() => {
                  setAddOpen(false);
                  setAddForm(emptyChecklistItem());
                  setAddError(null);
                }}
                disabled={adding}
              >
                Cancel
              </AppButton>
            </div>
          </form>
        ) : (
          <AppButton onClick={() => setAddOpen(true)}>Add an item</AppButton>
        )}
      </PageSection>

      <p><Link href={`/organize/${encodeURIComponent(eventId)}`}>Back to this event</Link></p>
    </Page>
  );
}
