...
function CoachMapPublicPageInner() {
  ...
  const loadMap = useCallback(async () => {
    try {
      ...
      // code + registration email/phone) sets this same local memberEvent
      // context but never creates a Supabase session and holds no
      // Participation link, so it is not eligible for that RPC -- it
      // resolves the same known Event id through the public known-ID
      // continuity path instead (get_event_continuity_context), already
      // anon+authenticated EXECUTE-granted and gated by the identical
      // visible_to_members/is_active/status predicate Temporary Event
      // Access admission itself already requires
      // (verify_member_event_login). Same pattern as the Nearby (d36ad11),
      // Locations (5fc355e), and /map (dc72034) reconciliations.
