...
function attendeeName(a: Attendee | undefined) {
  ...
  return [a.pilot_first, a.pilot_last].filter(Boolean).join(' ') || 'Unnamed attendee'
}
...
export default function CoachMapPage() {
  ...
  async function loadMapData(activeEventId: string) {
    const { data: attendeeData, error: attendeeError } = await supabase
      .from('attendees')
      .select('id,pilot_first,pilot_last,coach_make,coach_model,coach_length,share_with_attendees')
  ...
  const filteredSites = useMemo(() => {
    ...
    return sites.filter((site) => {
      ...
      const visibleName =
        assignedAttendee?.share_with_attendees
  ...
  const mapSites = useMemo(() => {
    return filteredSites.map((site) => {
      ...
      return {
        ...site,
        popupText: visibleOccupantLabel(assignedAttendee, !!site.assigned_attendee_id),
      }
    })
  ...
  }, [eventId, occupiedOnly, attendeeSearch, siteSearch, selectedSiteId, mapSites])
  ...
  const selectedCoachText =
    selectedAttendee && selectedAttendee.share_with_attendees
      ? [selectedAttendee.coach_make, selectedAttendee.coach_model].filter(Boolean).join(' ') || '—'
      : 'Private'
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      >
        <div>
          <div style={{ fontWeight: 700, marginBottom: 8 }}>Search Opted-In Attendee</div>
          <input
      ...
      {attendeeSearch.trim() && (
        ...
        >
          <div style={{ fontWeight: 700, marginBottom: 8 }}>Locator Results</div>
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      borderRadius: 8,
                      border: '1px solid #ddd',
                      background: selectedSiteId === site.id ? '#eef4ff' : 'white',
                      cursor: 'pointer',
                    }}
                  >
                    <strong>Site {site.site_number || '(no number)'}</strong> — {attendeeName(assigned)}
                  </button>
                )
              })}
            </div>
      ...
      {selectedSite && (
        ...
        >
          ...
          <div>Label: {selectedSite.display_label || selectedSite.site_number || '(none)'}</div>
          <div>Occupant: {selectedOccupantText}</div>
          <div>Coach: {selectedCoachText}</div>
          {selectedAttendee?.share_with_attendees && selectedAttendee?.coach_length && (
      ...
      <div style={{ marginBottom: 12, fontSize: 13, color: '#555' }}>
        Showing {filteredSites.length} site{filteredSites.length === 1 ? '' : 's'} on map.
      </div>
