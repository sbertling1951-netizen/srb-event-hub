...
**Correction, 2026-08-20 — scroll ownership (Web Interaction Standards Conformance Remediation).** This document originally recorded `.shell-root` as a fixed-height (`100vh`/`100dvh`) flex column making `.shell-main` "the single, real, bounded vertical scroll region for canonical routes — not the document" (Stage 2B §I). Real-device verification on physical iPhone/Safari — performed after ruling out both WebDriver-synthesized touch and OS-level automation-dialog interference as confounds, and independently confirmed with a real finger tap with no automation session attached — found this nested-scroll ownership fragments pinch-zoom panning into single-axis gestures: a real finger could pan vertically or horizontally after zooming in, but not continuously in both directions without lifting and re-touching. Tap activation itself was confirmed unaffected, both before and after zoom, ruling out a broader hit-testing defect.

The corrected rule: canonical routes use **native document scrolling**. `.shell-root` is `min-height: 100vh`/`100dvh` (grows with content, does not clip it), `.shell-main` no longer sets `overflow`, and `html` (`document.scrollingElement` in this app) is the sole scroll owner — the same model every other native web page uses, and the same model `LegacyChromeCompat` routes already used. `.shell-nav-desktop`'s previously-implicit "stays visible while content scrolls" behavior, which the old fixed-height row produced as a side effect, is now explicit: `position: sticky; top: 0` with its own `100vh`/`100dvh` height and `align-self: flex-start`. `.shell-header`'s existing `position: sticky; top: 0` is unchanged and now sticks relative to the document instead of `.shell-main`'s former inner scrollbox, which is the same visible behavior by a more standard mechanism.

**Follow-on discovery during the same remediation: a pre-existing, previously-latent sticky-positioning defect in the shared global reset.** Making `.shell-main` no longer a scroll container exposed a bug that predates this remediation entirely: the base reset gave both `html` and `body` a non-`visible` `overflow-x` while only `body` declared `overflow-y: auto` explicitly — but per the CSS Overflow Module's overflow-pairing rule, a non-`visible` value on one axis forces the other axis to compute as `auto` too, so `html` *also* computed `overflow-y: auto`, despite never being the element that actually scrolls (`document.scrollingElement` is `html`; `body.scrollTop` never moves). `position: sticky` resolves against the *nearest* ancestor with a non-`visible` overflow, so every sticky element in the app (`.shell-header`, `.shell-nav-desktop`, and `.table-toolbar` wherever it's used, e.g. Attendees/Vendors) was silently resolving against `body`'s non-moving scrollport instead of `html`'s real one and never actually stuck. This was inert before this remediation only because `.shell-main` (a genuine, correctly-functioning scroll container) was always the nearer ancestor for every canonical-shell sticky element, masking it. A third occurrence of the same root property (`.app-body`'s `overflow-x: hidden`, applied to `<body>` unconditionally by the root layout on every route, with a further `!important` duplicate inside the app's `@media (max-width: 899px)` compact-viewport block — i.e. active at phone widths) had to be found and corrected the same way. `html`'s own `overflow-x: hidden` already fully contains horizontal overflow for the whole page regardless of what `body` declares, so removing `body`'s and `.app-body`'s copies is redundant-restriction removal, not a behavior change to horizontal containment — confirmed by direct real-iPhone re-verification (sticky header now holds correctly while scrolling `/admin/dashboard`) after the fix.
