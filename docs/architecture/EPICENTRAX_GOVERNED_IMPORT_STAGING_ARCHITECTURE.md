...
`lib/importLifecycleOrchestration.ts` is the sole client orchestration module for the seven mutation/read RPCs above plus `get_import_run_status` -- import-type-agnostic (run lifecycle/History belong to the Imports domain, not to Attendee, Vendor, or Agenda specifically), shared by all governed doors rather than duplicated, and owning no lifecycle rule of its own: every state transition, terminality check, and authority decision is made server-side. Three shared UI components in `app/admin/imports/` wrap it and are mounted on the Attendee, Vendor, and Agenda doors (`app/admin/imports/page.tsx`, `VendorImportWorkflow.tsx`, and `app/admin/agenda/page.tsx`):

- **`ActiveRunsPanel`** discovers active (`staging`/`ready_for_review`) runs on the selected Event via `list_active_import_runs` (`event.imports.manage`-gated, Event-scoped server-side), narrowed client-side to the current door's own `import_type` for display only. Resume hands the run id back to the door's own existing `recoverAttendeeImportRun`/`recoverVendorImportRun`/`recoverAgendaImportRun` call -- this panel never recovers a run itself, so resume always goes through the same governed, authority-revalidating recovery path as the existing locator-based reload.
...
Remaining work deliberately outside the Attendee Stage 4 slice included the other import types; Vendor and Agenda have since moved onto this staging architecture in their separately governed stages. Data Review / Saved Attendee List redesign and broader Central UI Standard migration of `/admin/imports` remain separate work.
...
**Agenda** keeps one governed implementation -- browser file reading stays in `app/admin/agenda/page.tsx`, while its pure row interpretation and XLSX worksheet/header handling live once in `lib/agendaImportContract.ts`; Stage B then stages through the shared Imports architecture and commits through `commit_agenda_import_run`, requiring the composition of `event.imports.manage` and `event.agenda.manage`. The Imports `agenda` door does not embed a second importer; it is a template/instructions card whose "Open Agenda Import" action navigates to `/admin/agenda?mode=import`, which opens the exact same `AgendaImportPanel` Agenda's own "Import Agenda" tab already renders. Agenda's own page also has a reciprocal "Browse Imports" contextual link into `/admin/imports?type=agenda` -- two doors into one implementation, in both directions, with no authority substitution.
